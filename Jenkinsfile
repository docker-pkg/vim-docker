node {
	dir('vim-docker') {
			...
			checkout([
				...
				userRemoteConfigs: [
					[
						...
						url: 'https://github.com/docker-pkg/vim-docker.git',
					],
					[
						name: 'docker',
		...
		stage('Push Docker') {
			sh '''
				git push --force git@github.com:docker-pkg/vim-docker.git HEAD:refs/heads/docker
			'''
		}
		...
		stage('Push Neovim') {
			sh '''
				git push --force git@github.com:docker-pkg/vim-docker.git HEAD:refs/heads/neovim
			'''
		}
		...
		stage('Push Vim') {
			sh '''
				git push --force git@github.com:docker-pkg/vim-docker.git HEAD:refs/heads/vim
			'''
		}
	}
}
